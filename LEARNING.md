...
### 2026-05-11 (deep night) — 🏆🏆🏆 PHASE 5 + PROJECT 1 SHIPPED — admin portal live in production
- ✅ **M7.2 + M7.3 (merged)** — Real per-hostel lead table with inline editing:
  - `src/lib/api.ts` — typed `apiFetch<T>(path, options)` wrapper. Auto-attaches `Authorization: Bearer ${admin_token}` from localStorage. On 401: clears storage + `window.location.href = "/admin/login"` (the non-React escape hatch since wrapper isn't a component). Distinguishes network errors (status: 0) from HTTP errors. Generic over response type.
  - `src/hooks/useAdminLeads.ts` — custom hook. `{leads, loading, error, updateLead}`. Filters fetched leads to current hostel slug (client-side; fine for our scale). `updateLead(id, changes)` does optimistic local update → PATCH → on success syncs to server response, on failure reverts to previous state and re-throws. AbortController in useEffect cleanup.
  - `src/components/HostelLeadsShell.tsx` (rewrote as a table) — 7-column layout (#, Name, Contact, Action, Created, Status dropdown, Remarks input). Status dropdown writes via `updateLead`. Remarks saves on blur (only if changed). Per-row `SaveIndicator`: spinner → green ✓ (fades after 1.5s) → idle, or red ! on error.
- ✅ **M7.4** — Filters live: 30d/60d/90d/All segmented control (default 30d, saffron-filled when active), name search (case-insensitive substring), unactioned-only checkbox. All combined with AND via `useMemo([leads, timeWindow, search, unactionedOnly])`. Header count flips between "N total" and "X of Y leads" when filters active. Empty state differentiates "no leads yet for this hostel" from "no leads match these filters" (the latter shows a Clear filters button).
- ✅ **Design polish** — bumped text sizes (table base text-base, lead names text-xl Fraunces, headers text-xs tracked), padding `px-6 py-5`, full-bleed layout (no max-width cap), deeper cream background (`--color-cream-deep`) so the white table card pops.
- ✅ **M7.5 — deployed and verified end-to-end on production**:
  - First Vercel deploy failed: "two parallel pages that resolve to the same path" — the Route Group refactor commit added the new `(marketing)/blog/*` paths but didn't stage the deletions of the old `blog/*` paths. Local filesystem was clean (Move-Item removed them) but git still tracked both. Fixed by `git rm` on the old paths and a follow-up commit. Lesson: for restructural changes use `git add -A` or `git mv` (atomic), not explicit `git add <path>` which only catches what you name.
  - CORS: Render `ALLOWED_ORIGINS=http://localhost:3000,https://hostel-web-sand.vercel.app` (stable prod domain, not the per-commit deployment URLs which rotate).
  - `NEXT_PUBLIC_API_URL` on Vercel: confirmed pointing at Render backend. Without it, the prod frontend silently failed at the localhost fallback (mixed content blocked by browser).
  - First prod login: 30-90s cold start (Render free tier sleeps after 15 min idle). Subsequent: ~200ms. Concept: lazy initialization — trade first-access latency for $0 idle cost.
  - End-to-end smoke test green: login → dashboard → hostel card → table → status flip → remarks edit → filters → logout.
- 🪤 Production bug surfaced: button stuck on "Signing in…" with no error. Root cause: `NEXT_PUBLIC_API_URL` initially missing on Vercel → fallback hit `http://localhost:5000` → browser silently blocks http-from-https mixed content → fetch never resolves. Diagnosed via DevTools Network tab. Fix: set the env var on Vercel, redeploy.
- New concepts: **API client middleware pattern** (concierge analogy: auth attach + 401 logout + error normalize as decorator around `fetch`); **`typeof window !== "undefined"`** SSR-safety guard before touching browser APIs; **the non-React escape hatch** (`window.location.href` when you're outside a component); **the `{data, loading, error}` triple** as a 3-state state machine; **AbortController** for fetch cancellation + cleanup pattern in `useEffect` (destructor analogy); **Rules of Hooks** — call order, top-level only; **optimistic updates** (write-ahead log pattern: update UI first, server second, revert on failure — WhatsApp send analogy); **lift-state-via-the-hook** pattern (hook exposes mutators, no prop drilling — precursor to SWR/React Query); **`useMemo`** for memoizing computed values (mise en place analogy); **predicate composition** for multi-dimensional filters; **client-side vs server-side filtering** trade-off (push to SQL only when dataset grows); **Vercel per-commit deployment URLs** vs stable production domain; **mixed-content blocking** (https page → http resource silently dropped); **Render free-tier cold start** (lazy initialization in infra form); **CORS allowlist** (stable origins only, no wildcards without regex); **environment-variable hygiene at the platform boundary** — local `.env.local` doesn't reach prod, must set on Vercel separately.
...
- [x] **M7.2 + M7.3:** ✅ COMPLETE 2026-05-11 — Per-hostel lead table with inline editing. Custom hook `useAdminLeads(slug)` with optimistic `updateLead`, typed `apiFetch` wrapper with auto-Bearer + 401 auto-logout. Status dropdown + remarks input edit in-place via PATCH. Per-row SaveIndicator (spinner → check → idle, red ! on error).
- [x] **M7.4:** ✅ COMPLETE 2026-05-11 — Filters: 30/60/90/All time window (default 30), name search, unactioned-only toggle. All client-side via `useMemo`. Filtered count summary + "Clear filters" empty state.
- [x] **M7.5:** ✅ COMPLETE 2026-05-11 — Deployed to Vercel + Render, CORS allowlist verified, `NEXT_PUBLIC_API_URL` set, end-to-end smoke test passed on prod. **Project 1 functionally shipped.**

🎉 **PHASE 5 COMPLETE — PROJECT 1 IS LIVE.** Marketing site captures leads from real public traffic; admin portal lets staff manage them from any browser. Full-stack product end-to-end.

**Outstanding to-do (not blocking):**
- Rotate `ADMIN_PASSWORD` on Render before distributing to staff (current value `Vansh@123` is in chat history)
- Optional: set up keep-warm pinger at cron-job.org to ping Render backend every 10 min — eliminates the 30-90s cold start on first login of the day
- Reminder: Render free Postgres expires ~2026-08-08 — migrate to Neon or upgrade before then
