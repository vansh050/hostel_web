  ...
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50">
      ...
      {[...Array(6)].map((_, i) => (
        <motion.div
          key={`backpack-${i}`}
          className="absolute text-green-400 opacity-25"
          style={{
            left: `${15 + i * 15}%`,
            bottom: -60,
          }}
      ...
      {/* Pulsing location circles (like radar) */}
      {[...Array(5)].map((_, i) => (
        <motion.div
      ...
      {/* Stars for ratings/destinations */}
      {[...Array(30)].map((_, i) => (
        <motion.div
          key={`star-${i}`}
        ...
        >
          <Star size={12 + Math.random() * 8} fill="currentColor" strokeWidth={1} />
        </motion.div>
      ))}
      ...
      {[...Array(5)].map((_, i) => (
        <motion.div
          key={`wifi-${i}`}
          className="absolute text-cyan-400 opacity-25"
          style={{
            left: `${10 + i * 20}%`,
            top: `${20 + i * 15}%`,
          }}
      ...
      {/* Connecting dots (representing global connections) */}
